...
for ($year = 2020; $year <= 2023; $year++) {
  ...
  if($year == "2020"){
  $query_first_half = "SELECT * FROM application a WHERE YEAR(date_of_application) = $year AND MONTH(date_of_application) BETWEEN 1 AND 6 and listing_id = '$listing_id'";
  $result_first_half = mysqli_query($conn, $query_first_half);
  $total_renter_2020f = 0;
  ...
  if($year == "2020"){
  $query_second_half = "SELECT * FROM application WHERE YEAR(date_of_application) = $year AND MONTH(date_of_application) BETWEEN 7 AND 12 and listing_id = '$listing_id'";
  $result_second_half = mysqli_query($conn, $query_second_half);
  $total_renter_2020s = 0;
  ...
  while ($row_second_half = mysqli_fetch_assoc($result_second_half)) {
      ...
      $total_approved_2023s = $total_approved_count_second_half;
  }
  ...
  $sql1 = "SELECT * FROM listing where listing_id = '$listing_id'";
...
<!-- <div class="text-center">
  <?php echo "Occupancy Rate: " . '<span style="color:'.$rateColor.';font-size:18px;">' . number_format($occupancy_rate) . "%" ." ".$rateText. '</span> <br>'?>
  <?php echo "Previous Occupancy Rate: " . $accuracy_rate_prev . "%" ?>
</div>
<?php echo $total_2023?>
<br>
<?php echo $totalAdd_renter2?>
<br>
<?php echo $totalAdd_renter3?> 
...
<!-- <script>
    var options = {
        series: [{
            name: "Occupancy Rate",
            data: [<?php echo  $previousforecast1?>,<?php echo  $previousforecast2?>,<?php echo  $accuracy_rate_prev?>, <?php echo $rate_now?>]
        }],
        chart: {
            type: 'area',
            height: 350,
        ...
        labels: ["Jul - Dec 2022","Jan - Jun 2023","july - Dec 2023", "jan - june 2024"],
        xaxis: {
